        ...
        connection.beginTransaction((err) => {
            ...
            connection.query(query, [user_id, book_id], (error, results) => {
                ...
                if (results.length === 0) {
                    ...
                    const query = 'INSERT INTO `rating`(`book_id`, `user_id`, `stars`, `comment`, `date`) VALUES (?,?,?,?, NOW())'
                    connection.query(query, [book_id, user_id, rating, comment], (error, results) => {
                        if (error) {
                            // console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                            // return res.status(500).json({ message: 'Lỗi máy chủ' });
                        ...
                        connection.query(query, [book_id], (error, results) => {
                            ...
                            const query = 'UPDATE `books` SET `rating`= ?,`date`= NOW() WHERE id = ?'
                    ...
                    const query = 'UPDATE `rating` SET `stars`= ?, `comment`= ? WHERE user_id = ? and book_id = ?'
                    connection.query(query, [rating, comment, user_id, book_id], (error, results) => {
                        if (error) {
                            // console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                            // return res.status(500).json({ message: 'Lỗi máy chủ' });
                            console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
