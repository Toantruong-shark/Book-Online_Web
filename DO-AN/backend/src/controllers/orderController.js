...
class OrderController {
    ...
    getOrder(req, res) {
        ...
        connection.query(query, [req.session.user_id], (error, results) => {
            ...
            const query = 'SELECT `total_amount` FROM `orders` WHERE user_id = ? and status IS NULL'
            connection.query(query, [req.session.user_id], (error, results) => {
                if (error) {
                    console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                    return res.status(500).json({ message: 'Lỗi máy chủ' });
    ...
    getUserOrder(req, res) {
        ...
        const query = 'SELECT * FROM `orders` WHERE user_id = ? and status IS NOT NULL ORDER BY order_date DESC;'
    ...
    getOrderAll(req, res) {
        const query = 'SELECT * FROM `orders` WHERE status IS NOT NULL ORDER BY order_date DESC;'
                ...
                connection.query(query, [Number(id)], (error, results) => {
                    ...
                    if (results.length > 0) {
                        ...
                        for (var i = 0; i < book_quantity.length; i++) {
                            console.log(i);
                            (function (i) {
                                const query = 'UPDATE `books` SET `quantity_remaining`= quantity_remaining - ?,`quantity_sold`= quantity_sold + ? WHERE id = ?'
                                connection.query(query, [book_quantity[i].quantity, book_quantity[i].quantity, Number(book_quantity[i].book_id)], (error, results) => {
                                    if (error) {
                                        // console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                                        // return res.status(500).json({ message: 'Lỗi máy chủ' });
                                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
    ...
    dashboardStatus(req, res) {
        ...
        const getCountProducts = () => {
            return new Promise((resolve, reject) => {
                const query1 = "SELECT COUNT(title) as countProducts FROM `books` WHERE isDelete = false";
                connection.query(query1, (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        reject(error);
        ...
        Promise.all([getTotalSales(), getTotalOrder(), getCountProducts(), getTotalReturn()])
            .then(() => {
                res.status(200).json({
    ...
    getDetailOrderByUser(req, res) {
        ...
        const getListBook = () => {
            return new Promise((resolve, reject) => {
                ...
                connection.query(query, [req.session.user_id, order_id], (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        reject(error);
                    }
        ...
        const getTotalOrder = () => {
            return new Promise((resolve, reject) => {
                const query2 = 'SELECT `order_date`, `total_amount`, `status`, `name_des`, `address_des`, `phone_des` FROM `orders` WHERE id = ? and status IS NOT NULL and user_id = ?';
                connection.query(query2, [order_id, req.session.user_id], (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        reject(error);
                    }
